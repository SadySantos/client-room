...
public class RoomController {
    ...
    @PutMapping("/rooms/{id}")
    public ResponseEntity<Room> updateRooms(@PathVariable (value = "id") Long roomId,
                                        @Valid @RequestBody Room roomDetais) throws ResourceNotFoundException{
        Room room = roomRepository.findById(roomid)
                .orElseThrow(()-> new ResourceNotFoundException("Room not found for this id::"+ roomId));
        room.setName(roomDetais.getName());
        room.setDate(roomDetais.getDate());
        room.setStartHour(roomDetais.getStartHour());
        room.setEndHour(roomDetais.getEndHour());
